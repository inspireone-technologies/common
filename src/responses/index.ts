...
export const paginatedResponse = (res: Response, data: any, pagination: object, message = 'Success') => {
...
export const multiStatusResponse = async(res: Response, data:Array<{status:number, message:string, [key:string]:any}>=[]) => {
	return res.status(ResponseStatus.MULTI_STATUS).json(data);
}